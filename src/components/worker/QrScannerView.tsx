...
export function QrScannerView({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Успешный скан (handled=true) уже сам дождался stop()+clear() перед
      // onScan()/навигацией (см. комментарий выше в success-callback) —
      // сканер к этому моменту гарантированно уже остановлен. Вызывать
      // stop() второй раз здесь не нужно и опасно: html5-qrcode.stop()
      // кидает СИНХРОННОЕ исключение (обычную строку, не Promise-реджект),
      // если сканер не в состоянии SCANNING/PAUSED — .catch() ниже от
      // этого не защищает, он навешен на результат stop(), а stop() в
      // этом случае ничего не возвращает, просто бросает до того, как
      // успевает создаться промис. Это и есть причина, по которой первый
      // переход /scan/coil → /coil/:id после успешного скана иногда падал
      // в Error Boundary ("Reload Page") — необработанное синхронное
      // исключение в cleanup эффекта при размонтировании во время
      // навигации (см. Decision Log ARCHITECTURE.md §15).
      if (handled) return;
      ...
      try {
        ...
        // Тот же случай синхронного throw, что и выше, но для пути "скана
        // не было" (отмена/размонтирование до handled=true) — например,
        // start() ещё не успел перевести сканер в SCANNING. Тут это
        // ожидаемо и безопасно: чистить нечего, сканер и так не запущен.
      }
    };
